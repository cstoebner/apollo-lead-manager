...
function LeadTable({ leads, onSelect }: { leads: Lead[]; onSelect: (id: string) => void }) {
  ...
  const filteredLeads = normalizedQuery
    ? leads.filter((lead) => `${lead.name} ${lead.studentName ?? ''} ${lead.instruments.join(' ')} ${lead.source} ${lead.campaign}`.toLowerCase().includes(normalizedQuery))
    : leads
...
function ScheduleEntryEditor({ leads, instructor, slot, onClose, onSave, onDelete, onSkipDate }: {
  ...
  const rawInstrumentOptions = selectedLead ? instructor.instruments.filter((item) => selectedLead.instruments.includes(item)) : instructor.instruments
  const instrumentOptions = rawInstrumentOptions.length ? rawInstrumentOptions : instructor.instruments
